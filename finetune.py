...
# Začátek trénovacího skriptu
    ...
    my_weight_path = (r"C:\Users\USER\Desktop\weights\dice_loss_constant_lr_e50_11200len.pth")
    net = smp.Unet(encoder_name="resnet34", encoder_weights=None, in_channels=3, classes=1, activation=None)
    net = net.to(device)
    my_weights = torch.load(my_weight_path, map_location=device, weights_only=True)
    net.load_state_dict(my_weights)

    optimizer = optim.AdamW(net.parameters(), lr=0.0001, weight_decay=1e-5) #weight_decay=1e-5
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[10], gamma=0.1)
    scheduler_2 = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="max", patience=5, factor=0.1, threshold=0.001)
    ...
    tversky_loss = TverskyLoss(mode="binary", from_logits=True, smooth=1e-6, alpha=0.8, beta=0.2)

    current_best_dice = float("-inf")
    ...
    for epoch in range(epochs):
        ...
        if epoch >= 10 and epoch % 10 == 0:
            current_model_save_path = rf"C:\Users\USER\Desktop\weights\finetuned_e"+str(epoch)+"_11200len.pth"
            ...
            model_and_metadata = {
                ...
                "epochs": epochs,
                "lr": 0.0001, 
                "train_dataset_len": 11200,
                "train_dice": smp_f1_train,
                "train_iou": smp_iou_train,
                "train_precision": smp_precision_train,
                "train_recall": smp_recall_train,

                "loss_function": "tversky loss (alpha=0.8, beta=0.2)",
                "augmentation": "my augmentation",
                "wanted_level": 2,
                "model_state_dict": net.state_dict()
            }
            # Uložení váh modelu
            torch.save(model_and_metadata, current_model_save_path)
            print(f"Model byl uložen do {current_model_save_path}")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(r'C:\Users\USER\Desktop\finetuned_dice_loss_e10_14400len.png')
    ...
    model_save_path = r"C:\Users\USER\Desktop\weights\finetuned_tversky_loss_e20_11200len.pth"
