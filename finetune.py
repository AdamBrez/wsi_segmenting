...
# Začátek trénovacího skriptu
    ...
    model_data = torch.load(r"C:\Users\USER\Desktop\results\2025-05-13_21-11-56\final_weights_2025-05-13_21-11-56.pth", map_location=device, weights_only=False) 
    ...
    focal_loss = FocalLoss(mode="binary", alpha=0.5, gamma=2.0)
    tversky_loss = TverskyLoss(mode="binary", alpha=0.7, beta=0.3, from_logits=True)
    ...
    for epoch in range(epochs):
        ...
        for k,(data,lbl) in enumerate(trainloader):
            ...
            net.train()
            output = net(data)
            loss_f = focal_loss(output, lbl)
            loss_t = tversky_loss(output, lbl)
            loss = loss_f + loss_t
    ...
    model_and_metadata = {
        ...
        "loss_function": "FocalLoss + TverskyLoss",
